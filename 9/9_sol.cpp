...
std::uint32_t get_num_visited_fields(RopeField &field, const std::vector<MoveOp> &moves, std::int32_t x=0, std::int32_t y=0);
...
std::uint32_t day_9_1(const std::string &file_path)
{
    ...
    return get_num_visited_fields(field, move_ops, 500, 500);
}
...
std::uint32_t get_num_visited_fields(RopeField &field, const std::vector<MoveOp> &moves, std::int32_t h_x, std::int32_t h_y)
{
    std::uint32_t num_v{ 1u };
    std::int32_t t_x{ h_x }; // tail x pos
    std::int32_t t_y{ h_y }; // tail y pos
    field[t_x][t_y] = true;
